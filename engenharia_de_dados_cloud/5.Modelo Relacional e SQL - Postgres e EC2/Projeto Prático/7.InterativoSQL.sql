...
select idvendedor, sum(total) from relacional.vendas
group by idvendedor;
--TOTAL DE VENDAS POR VENDEDOR
select idvendedor, sum(total) from relacional.vendas
where idvendedor = 1
...
select nome,data,total  from relacional.vendas as vendas
inner join relacional.vendedores as vendedores  on(vendas.idvendedor = vendedores.idvendedor );

select nome as vendedor,cliente, data,total  from relacional.vendas as vendas
inner join relacional.vendedores as vendedores  on(vendas.idvendedor = vendedores.idvendedor )
inner join relacional.clientes as clientes  on(vendas.idcliente = clientes.idcliente );


select vendedores.nome, sum(total)  from relacional.vendas as vendas
inner join relacional.vendedores as vendedores  on(vendas.idvendedor = vendedores.idvendedor )
